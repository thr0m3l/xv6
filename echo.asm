...
char *gets(char *buf, int max) {
 ...
 1a5:	57                   	push   %edi
 1a6:	6a 00                	push   $0x0
 1a8:	e8 4e 01 00 00       	call   2fb <read>
    if (cc < 1)
...
int stat(const char *n, struct stat *st) {
 ...
 20d:	e8 11 01 00 00       	call   323 <open>
  if (fd < 0)
 ...
 221:	50                   	push   %eax
 222:	e8 14 01 00 00       	call   33b <fstat>
  close(fd);
 227:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 22a:	89 c6                	mov    %eax,%esi
  close(fd);
 22c:	e8 da 00 00 00       	call   30b <close>
  return r;
...
int atoi(const char *s) {
 ...
 25c:	53                   	push   %ebx
  ...
  nbr = 0;
  if (s[0] == '-') {
 25d:	0f b6 11             	movzbl (%ecx),%edx
 260:	80 fa 2d             	cmp    $0x2d,%dl
 263:	75 0c                	jne    271 <atoi+0x21>
    sign = -1;
    s++;
  }
  i = 0;
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 265:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 269:	be ff ff ff ff       	mov    $0xffffffff,%esi
    s++;
 26e:	83 c1 01             	add    $0x1,%ecx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 271:	8d 5a d0             	lea    -0x30(%edx),%ebx
 274:	80 fb 09             	cmp    $0x9,%bl
 277:	77 2f                	ja     2a8 <atoi+0x58>
 279:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 27c:	31 db                	xor    %ebx,%ebx
 27e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 280:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 283:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 286:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 289:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 28c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 28f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 293:	8d 42 d0             	lea    -0x30(%edx),%eax
 296:	3c 09                	cmp    $0x9,%al
 298:	76 e6                	jbe    280 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 29a:	89 f0                	mov    %esi,%eax
 29c:	0f af c3             	imul   %ebx,%eax
}
 29f:	5b                   	pop    %ebx
 2a0:	5e                   	pop    %esi
 2a1:	5d                   	pop    %ebp
 ...
 611:	a1 c8 0a 00 00       	mov    0xac8,%eax
{
 ...
 651:	5b                   	pop    %ebx
  freep = p;
 652:	89 15 c8 0a 00 00    	mov    %edx,0xac8
}
 658:	5e                   	pop    %esi
 659:	5f                   	pop    %edi
 65a:	5d                   	pop    %ebp
 ...
 689:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 68c:	89 15 c8 0a 00 00    	mov    %edx,0xac8
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 6a9:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 6ac:	8b 3d c8 0a 00 00    	mov    0xac8,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6ef:	73 4f                	jae    740 <malloc+0xa0>
    ...
    if(p == freep)
 6f1:	8b 3d c8 0a 00 00    	mov    0xac8,%edi
 ...
 71a:	e8 f1 fe ff ff       	call   610 <free>
  return freep;
 71f:	8b 15 c8 0a 00 00    	mov    0xac8,%edx
 ...
 74c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 74f:	89 15 c8 0a 00 00    	mov    %edx,0xac8
}
 ...
 767:	0a 00 00 
    base.s.size = 0;
 76a:	bf cc 0a 00 00       	mov    $0xacc,%edi
    base.s.ptr = freep = prevp = &base;
 76f:	c7 05 cc 0a 00 00 cc 	movl   $0xacc,0xacc
 776:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 779:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 77b:	c7 05 d0 0a 00 00 00 	movl   $0x0,0xad0
 782:	00 00 00 
    if(p->s.size >= nunits){
