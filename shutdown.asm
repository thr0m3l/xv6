...
int main(int argc, char *argv[]) {
  ...
  shutdown();
  1d:	e8 21 03 00 00       	call   343 <shutdown>
  exit();
  22:	e8 7c 02 00 00       	call   2a3 <exit>
  27:	66 90                	xchg   %ax,%ax
  29:	66 90                	xchg   %ax,%ax
  2b:	66 90                	xchg   %ax,%ax
  2d:	66 90                	xchg   %ax,%ax
...
int stat(const char *n, struct stat *st) {
 ...
 1cd:	e8 11 01 00 00       	call   2e3 <open>
  if (fd < 0)
 ...
 1e1:	50                   	push   %eax
 1e2:	e8 14 01 00 00       	call   2fb <fstat>
  close(fd);
 1e7:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 1ea:	89 c6                	mov    %eax,%esi
  close(fd);
 1ec:	e8 da 00 00 00       	call   2cb <close>
  return r;
...
int atoi(const char *s) {
 ...
 21c:	53                   	push   %ebx
  ...
  nbr = 0;
  if (s[0] == '-') {
 21d:	0f b6 11             	movzbl (%ecx),%edx
 220:	80 fa 2d             	cmp    $0x2d,%dl
 223:	75 0c                	jne    231 <atoi+0x21>
    sign = -1;
    s++;
  }
  i = 0;
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 225:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 229:	be ff ff ff ff       	mov    $0xffffffff,%esi
    s++;
 22e:	83 c1 01             	add    $0x1,%ecx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 231:	8d 5a d0             	lea    -0x30(%edx),%ebx
 234:	80 fb 09             	cmp    $0x9,%bl
 237:	77 2f                	ja     268 <atoi+0x58>
 239:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 23c:	31 db                	xor    %ebx,%ebx
 23e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 240:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 243:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 246:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 249:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 24c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 24f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 253:	8d 42 d0             	lea    -0x30(%edx),%eax
 256:	3c 09                	cmp    $0x9,%al
 258:	76 e6                	jbe    240 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 25a:	89 f0                	mov    %esi,%eax
 25c:	0f af c3             	imul   %ebx,%eax
}
 25f:	5b                   	pop    %ebx
 260:	5e                   	pop    %esi
 261:	5d                   	pop    %ebp
 ...
 5d1:	a1 88 0a 00 00       	mov    0xa88,%eax
{
 ...
 649:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 64c:	89 15 88 0a 00 00    	mov    %edx,0xa88
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 669:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 66c:	8b 3d 88 0a 00 00    	mov    0xa88,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6af:	73 4f                	jae    700 <malloc+0xa0>
    ...
    if(p == freep)
 6b1:	8b 3d 88 0a 00 00    	mov    0xa88,%edi
 ...
 6da:	e8 f1 fe ff ff       	call   5d0 <free>
  return freep;
 6df:	8b 15 88 0a 00 00    	mov    0xa88,%edx
 ...
 727:	0a 00 00 
    base.s.size = 0;
 72a:	bf 8c 0a 00 00       	mov    $0xa8c,%edi
    base.s.ptr = freep = prevp = &base;
 ...
 739:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 73b:	c7 05 90 0a 00 00 00 	movl   $0x0,0xa90
 742:	00 00 00 
    if(p->s.size >= nunits){
