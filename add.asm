...
int main(int argc, char *argv[]) {
  ...
  19:	6a 08                	push   $0x8
int main(int argc, char *argv[]) {
  1b:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  struct mystat *ct = malloc(sizeof(struct mystat));
  1e:	e8 8d 06 00 00       	call   6b0 <malloc>
  ct->sz = argc - 1;
  int i;
  for (i = 1; i < argc; i++) {
  ...
  ct->sz = argc - 1;
  28:	8d 43 ff             	lea    -0x1(%ebx),%eax
  2b:	89 45 e0             	mov    %eax,-0x20(%ebp)
  2e:	89 46 04             	mov    %eax,0x4(%esi)
  for (i = 1; i < argc; i++) {
  31:	83 fb 01             	cmp    $0x1,%ebx
  34:	7e 2b                	jle    61 <main+0x61>
  36:	31 db                	xor    %ebx,%ebx
  38:	8d b4 26 00 00 00 00 	lea    0x0(%esi,%eiz,1),%esi
  3f:	90                   	nop
    // printf(1,"%d->" , atoi(argv[i]));
    ct->nums[i - 1] = atoi(argv[i]);
  40:	8b 06                	mov    (%esi),%eax
  42:	83 ec 0c             	sub    $0xc,%esp
  45:	8d 3c 98             	lea    (%eax,%ebx,4),%edi
  48:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  4b:	ff 74 98 04          	push   0x4(%eax,%ebx,4)
  for (i = 1; i < argc; i++) {
  4f:	83 c3 01             	add    $0x1,%ebx
    ct->nums[i - 1] = atoi(argv[i]);
  52:	e8 09 02 00 00       	call   260 <atoi>
  for (i = 1; i < argc; i++) {
  57:	83 c4 10             	add    $0x10,%esp
    ct->nums[i - 1] = atoi(argv[i]);
  5a:	89 07                	mov    %eax,(%edi)
  for (i = 1; i < argc; i++) {
  5c:	39 5d e0             	cmp    %ebx,-0x20(%ebp)
  5f:	75 df                	jne    40 <main+0x40>
  }
  printf(1, "%d\n", add(ct));
  61:	83 ec 0c             	sub    $0xc,%esp
  64:	56                   	push   %esi
  65:	e8 39 03 00 00       	call   3a3 <add>
  6a:	83 c4 0c             	add    $0xc,%esp
  6d:	50                   	push   %eax
  6e:	68 a8 07 00 00       	push   $0x7a8
  73:	6a 01                	push   $0x1
...
char *gets(char *buf, int max) {
 ...
 1b5:	57                   	push   %edi
 1b6:	6a 00                	push   $0x0
 1b8:	e8 4e 01 00 00       	call   30b <read>
    if (cc < 1)
...
int stat(const char *n, struct stat *st) {
 ...
 21d:	e8 11 01 00 00       	call   333 <open>
  if (fd < 0)
 ...
 231:	50                   	push   %eax
 232:	e8 14 01 00 00       	call   34b <fstat>
  close(fd);
 237:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 23a:	89 c6                	mov    %eax,%esi
  close(fd);
 23c:	e8 da 00 00 00       	call   31b <close>
  return r;
...
int atoi(const char *s) {
 ...
 263:	56                   	push   %esi
 264:	be 01 00 00 00       	mov    $0x1,%esi
 ...
 275:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 ...
 287:	77 2f                	ja     2b8 <atoi+0x58>
 289:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 28c:	31 db                	xor    %ebx,%ebx
 28e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 290:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 293:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 296:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 299:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 29c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 29f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 2a3:	8d 42 d0             	lea    -0x30(%edx),%eax
 2a6:	3c 09                	cmp    $0x9,%al
 2a8:	76 e6                	jbe    290 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 2aa:	89 f0                	mov    %esi,%eax
 2ac:	0f af c3             	imul   %ebx,%eax
}
 2af:	5b                   	pop    %ebx
 2b0:	5e                   	pop    %esi
 2b1:	5d                   	pop    %ebp
 ...
 489:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 607:	90                   	nop
          s = "(null)";
 608:	ba ac 07 00 00       	mov    $0x7ac,%edx
        while(*s != 0){
