...
char *gets(char *buf, int max) {
 ...
 195:	57                   	push   %edi
 196:	6a 00                	push   $0x0
 198:	e8 4e 01 00 00       	call   2eb <read>
    if (cc < 1)
...
int stat(const char *n, struct stat *st) {
 ...
 1fd:	e8 11 01 00 00       	call   313 <open>
  if (fd < 0)
 ...
 211:	50                   	push   %eax
 212:	e8 14 01 00 00       	call   32b <fstat>
  close(fd);
 217:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 21a:	89 c6                	mov    %eax,%esi
  close(fd);
 21c:	e8 da 00 00 00       	call   2fb <close>
  return r;
...
int atoi(const char *s) {
 ...
 243:	56                   	push   %esi
 244:	be 01 00 00 00       	mov    $0x1,%esi
 ...
 250:	80 fa 2d             	cmp    $0x2d,%dl
 253:	75 0c                	jne    261 <atoi+0x21>
    sign = -1;
    s++;
  }
  i = 0;
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 255:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 259:	be ff ff ff ff       	mov    $0xffffffff,%esi
    s++;
 25e:	83 c1 01             	add    $0x1,%ecx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 261:	8d 5a d0             	lea    -0x30(%edx),%ebx
 264:	80 fb 09             	cmp    $0x9,%bl
 267:	77 2f                	ja     298 <atoi+0x58>
 269:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 26c:	31 db                	xor    %ebx,%ebx
 26e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 270:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 273:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 276:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 279:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 27c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 27f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 283:	8d 42 d0             	lea    -0x30(%edx),%eax
 286:	3c 09                	cmp    $0x9,%al
 288:	76 e6                	jbe    270 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 28a:	89 f0                	mov    %esi,%eax
 28c:	0f af c3             	imul   %ebx,%eax
}
 28f:	5b                   	pop    %ebx
 290:	5e                   	pop    %esi
 291:	5d                   	pop    %ebp
 ...
 601:	a1 d0 0a 00 00       	mov    0xad0,%eax
{
 ...
 641:	5b                   	pop    %ebx
  freep = p;
 642:	89 15 d0 0a 00 00    	mov    %edx,0xad0
}
 648:	5e                   	pop    %esi
 649:	5f                   	pop    %edi
 64a:	5d                   	pop    %ebp
 ...
 679:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 67c:	89 15 d0 0a 00 00    	mov    %edx,0xad0
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 699:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 69c:	8b 3d d0 0a 00 00    	mov    0xad0,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 6df:	73 4f                	jae    730 <malloc+0xa0>
    ...
    if(p == freep)
 6e1:	8b 3d d0 0a 00 00    	mov    0xad0,%edi
 ...
 70a:	e8 f1 fe ff ff       	call   600 <free>
  return freep;
 70f:	8b 15 d0 0a 00 00    	mov    0xad0,%edx
 ...
 73c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 73f:	89 15 d0 0a 00 00    	mov    %edx,0xad0
}
 ...
 74f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 750:	c7 05 d0 0a 00 00 d4 	movl   $0xad4,0xad0
 757:	0a 00 00 
    base.s.size = 0;
 75a:	bf d4 0a 00 00       	mov    $0xad4,%edi
    base.s.ptr = freep = prevp = &base;
 75f:	c7 05 d4 0a 00 00 d4 	movl   $0xad4,0xad4
 766:	0a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 769:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 76b:	c7 05 d8 0a 00 00 00 	movl   $0x0,0xad8
 772:	00 00 00 
    if(p->s.size >= nunits){
