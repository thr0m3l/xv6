...
char *gets(char *buf, int max) {
 ...
 275:	57                   	push   %edi
 276:	6a 00                	push   $0x0
 278:	e8 4e 01 00 00       	call   3cb <read>
    if (cc < 1)
...
int stat(const char *n, struct stat *st) {
 ...
 2dd:	e8 11 01 00 00       	call   3f3 <open>
  if (fd < 0)
 ...
 2f1:	50                   	push   %eax
 2f2:	e8 14 01 00 00       	call   40b <fstat>
  close(fd);
 2f7:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 2fa:	89 c6                	mov    %eax,%esi
  close(fd);
 2fc:	e8 da 00 00 00       	call   3db <close>
  return r;
...
int atoi(const char *s) {
 ...
 323:	56                   	push   %esi
 324:	be 01 00 00 00       	mov    $0x1,%esi
 ...
 335:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 339:	be ff ff ff ff       	mov    $0xffffffff,%esi
    s++;
 33e:	83 c1 01             	add    $0x1,%ecx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 341:	8d 5a d0             	lea    -0x30(%edx),%ebx
 344:	80 fb 09             	cmp    $0x9,%bl
 347:	77 2f                	ja     378 <atoi+0x58>
 349:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 34c:	31 db                	xor    %ebx,%ebx
 34e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 350:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 353:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 356:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 359:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 35c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 35f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 363:	8d 42 d0             	lea    -0x30(%edx),%eax
 366:	3c 09                	cmp    $0x9,%al
 368:	76 e6                	jbe    350 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 36a:	89 f0                	mov    %esi,%eax
 36c:	0f af c3             	imul   %ebx,%eax
}
 36f:	5b                   	pop    %ebx
 370:	5e                   	pop    %esi
 371:	5d                   	pop    %ebp
...
free(void *ap)
{
 ...
 6e1:	a1 b8 0b 00 00       	mov    0xbb8,%eax
{
 ...
 721:	5b                   	pop    %ebx
  freep = p;
 722:	89 15 b8 0b 00 00    	mov    %edx,0xbb8
}
 728:	5e                   	pop    %esi
 729:	5f                   	pop    %edi
 72a:	5d                   	pop    %ebp
 ...
 759:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 75c:	89 15 b8 0b 00 00    	mov    %edx,0xbb8
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 779:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 77c:	8b 3d b8 0b 00 00    	mov    0xbb8,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 7bf:	73 4f                	jae    810 <malloc+0xa0>
    ...
    if(p == freep)
 7c1:	8b 3d b8 0b 00 00    	mov    0xbb8,%edi
 ...
 7ea:	e8 f1 fe ff ff       	call   6e0 <free>
  return freep;
 7ef:	8b 15 b8 0b 00 00    	mov    0xbb8,%edx
 ...
 82f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 830:	c7 05 b8 0b 00 00 bc 	movl   $0xbbc,0xbb8
 837:	0b 00 00 
    base.s.size = 0;
 83a:	bf bc 0b 00 00       	mov    $0xbbc,%edi
    base.s.ptr = freep = prevp = &base;
 83f:	c7 05 bc 0b 00 00 bc 	movl   $0xbbc,0xbbc
 846:	0b 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 849:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 84b:	c7 05 c0 0b 00 00 00 	movl   $0x0,0xbc0
 852:	00 00 00 
    if(p->s.size >= nunits){
