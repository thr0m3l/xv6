...
char *gets(char *buf, int max) {
 ...
 2b5:	57                   	push   %edi
 2b6:	6a 00                	push   $0x0
 2b8:	e8 4e 01 00 00       	call   40b <read>
    if (cc < 1)
...
int stat(const char *n, struct stat *st) {
 ...
 31d:	e8 11 01 00 00       	call   433 <open>
  if (fd < 0)
 ...
 331:	50                   	push   %eax
 332:	e8 14 01 00 00       	call   44b <fstat>
  close(fd);
 337:	89 1c 24             	mov    %ebx,(%esp)
  r = fstat(fd, st);
 33a:	89 c6                	mov    %eax,%esi
  close(fd);
 33c:	e8 da 00 00 00       	call   41b <close>
  return r;
...
int atoi(const char *s) {
 ...
 363:	56                   	push   %esi
 364:	be 01 00 00 00       	mov    $0x1,%esi
 ...
 375:	0f b6 51 01          	movzbl 0x1(%ecx),%edx
    sign = -1;
 379:	be ff ff ff ff       	mov    $0xffffffff,%esi
    s++;
 37e:	83 c1 01             	add    $0x1,%ecx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 381:	8d 5a d0             	lea    -0x30(%edx),%ebx
 384:	80 fb 09             	cmp    $0x9,%bl
 387:	77 2f                	ja     3b8 <atoi+0x58>
 389:	83 c1 01             	add    $0x1,%ecx
  val = 0;
 38c:	31 db                	xor    %ebx,%ebx
 38e:	66 90                	xchg   %ax,%ax
    nbr = (int)(s[i] - '0');
 390:	83 ea 30             	sub    $0x30,%edx
    val = (val * 10) + nbr;
 393:	8d 1c 9b             	lea    (%ebx,%ebx,4),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 396:	83 c1 01             	add    $0x1,%ecx
    nbr = (int)(s[i] - '0');
 399:	0f be d2             	movsbl %dl,%edx
    val = (val * 10) + nbr;
 39c:	8d 1c 5a             	lea    (%edx,%ebx,2),%ebx
  while (s[i] >= '0' && s[i] <= '9' && s[i] != '\0') {
 39f:	0f b6 51 ff          	movzbl -0x1(%ecx),%edx
 3a3:	8d 42 d0             	lea    -0x30(%edx),%eax
 3a6:	3c 09                	cmp    $0x9,%al
 3a8:	76 e6                	jbe    390 <atoi+0x30>
    i++;
  }
  i++;
  return (val * sign);
 3aa:	89 f0                	mov    %esi,%eax
 3ac:	0f af c3             	imul   %ebx,%eax
}
 3af:	5b                   	pop    %ebx
 3b0:	5e                   	pop    %esi
 3b1:	5d                   	pop    %ebp
